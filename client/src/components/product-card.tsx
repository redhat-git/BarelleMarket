...
import { Link } from "wouter"; // Importez Link de wouter
...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  const handleAddToCart = (e: React.MouseEvent) => {
    e.preventDefault(); // Empêche la propagation du clic vers le Link parent
  ...
  return (
    <Link href={`/products/${product.slug}`}> {/* Ajout du Link autour de la carte */}
      <Card className="group relative overflow-hidden border-0 shadow-lg hover:shadow-xl transition-all duration-300 bg-white w-full max-w-sm mx-auto cursor-pointer">
        <div
