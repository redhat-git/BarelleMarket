...
export default function AdminOrders() {
  ...
  const { data: ordersData, isLoading, error } = useQuery<{ orders: Order[]; total: number }>({
    ...
    queryFn: async () => {
      ...
      try {
        // Option 1: Si apiRequest attend une méthode et une URL
        const result = await apiRequest('GET', url);
        console.log('Orders fetched:', result);
        return result;
      } catch (err) {
        console.log('apiRequest failed, trying fetch directly:', err);
        // Option 2: Utiliser fetch directement si apiRequest ne fonctionne pas
        const response = await fetch(url, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
            // Ajoutez ici d'autres headers si nécessaire (auth, etc.)
          },
        });
        ...
        console.log('Orders fetched with fetch:', result);
        return result;
      }
    },
    retry: 3,
  ...
  const { data: orderDetails } = useQuery<OrderWithItems>({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        console.log('apiRequest failed for order details, trying fetch:', err);
        // Option 2: Utiliser fetch directement
        const response = await fetch(url, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
        ...
        const result = await response.json();
        console.log('Order details fetched with fetch:', result);
        return result;
      }
    },
  });
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className="mb-4 p-3 bg-blue-50 rounded text-sm">
              ...
              {ordersData && !ordersData.orders && <span className="text-red-600"> - Structure de données incorrecte</span>}
            </div>
                    ...
                    {ordersData.orders.map((order) => (
                      <tr key={order.id} className="border-b hover:bg-gray-50">
                        ...
                        <td className="py-3 px-4 hidden sm:table-cell">
                          <div>
                            <div className="font-medium">{order.customerName || 'Nom non disponible'}</div>
                            <div className="text-sm text-gray-500">{order.customerEmail || 'Email non disponible'}</div>
                          </div>
                        </td>
                        <td className="py-3 px-4 font-medium">
                          {order.total ? parseFloat(order.total.toString()).toLocaleString() : '0'} CFA
                        </td>
                          ...
                          <div className="flex gap-2">
                            <Dialog>
                              ...
                              <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
                                <DialogHeader>
                                  <DialogTitle>Détails de la Commande {order.orderNumber || `#${order.id}`}</DialogTitle>
                                </DialogHeader>
                                {orderDetails && (
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                      <Card>
                                        ...
                                        <CardContent className="space-y-2">
                                          <p><strong>Nom:</strong> {orderDetails.customerName || 'Non renseigné'}</p>
                                          <p><strong>Email:</strong> {orderDetails.customerEmail || 'Non renseigné'}</p>
                                          <p><strong>Téléphone:</strong> {orderDetails.customerPhone || 'Non renseigné'}</p>
                                          <p><strong>Type:</strong> {orderDetails.customerType || 'B2C'}</p>
                                        </CardContent>
                                      </Card>
                                      ...
                                      <Card>
                                        ...
                                        <CardContent className="space-y-2">
                                          <p><strong>Adresse:</strong> {orderDetails.deliveryAddress || 'Non renseignée'}</p>
                                          <p><strong>Ville:</strong> {orderDetails.deliveryCity || 'Non renseignée'}</p>
                                      ...
                                      <CardContent>
                                          ...
                                          {orderDetails.orderItems?.map((item) => (
                                            <div key={item.id} className="flex justify-between items-center p-3 bg-gray-50 rounded">
                                              ...
                                              <div className="text-right">
                                                ...
                                                <p className="text-sm text-gray-600">{parseFloat(item.productPrice?.toString() || '0').toLocaleString()} CFA/unité</p>
                                              </div>
                                            </div>
                                          ))}
                                        </div>
                                        <div className="mt-4 pt-4 border-t space-y-2">
                                          <div className="flex justify-between">
                                            ...
                                            <span>{parseFloat(orderDetails.subtotal?.toString() || '0').toLocaleString()} CFA</span>
                                          </div>
                                          <div className="flex justify-between">
                                            <span>Frais de livraison:</span>
                                            <span>{parseFloat(orderDetails.deliveryFee?.toString() || '0').toLocaleString()} CFA</span>
                                          </div>
                                          <div className="flex justify-between font-bold text-lg">
                                            <span>Total:</span>
                                            <span>{parseFloat(orderDetails.total?.toString() || '0').toLocaleString()} CFA</span>
                                          </div>
                                        </div>
                                      </CardContent>
                                    </Card>
